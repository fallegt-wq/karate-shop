...
export default function Checkout() {
  ...
  const [agreeTerms, setAgreeTerms] = useState(false);// src/pages/Checkout.jsx
import React, { useEffect, useMemo, useState } from "react";
import { Link, useLocation, useParams } from "react-router-dom";
import { useCart } from "../context/CartContext";
import ClubShell from "../components/layout/ClubShell";
...
const SHIPPING_ENABLED_FOR_CLUB = true;
const CHECKOUT_PENDING_STORAGE_KEY = "checkout_pending_order";
...
export default function Checkout() {
  ...
  const validationMessage = useMemo(() => {
    ...
    if (deliveryMethod === "SHIP") {
      if (!shippingAddress.street.trim()) return "Vantar heimilisfang.";
      if (!shippingAddress.postalCode.trim()) return "Vantar póstnúmer.";
      if (!shippingAddress.city.trim()) return "Vantar bæjarfélag.";
    }
  ...
  async function submitOrder() {
    ...
    const orderPayload = {
      ...
      delivery: {
        ...
        label: deliveryMethod === "SHIP" ? "Fá sent heim" : "Sótt hjá félagi",
        shippingEnabledForClub: SHIPPING_ENABLED_FOR_CLUB,
        address:
          deliveryMethod === "SHIP"
            ? {
    ...
    >
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
        <section className="space-y-6 lg:col-span-7">
          ...
          <div className="rounded-2xl border bg-white p-6 shadow-sm">
            <div className="text-xs uppercase tracking-wider text-zinc-500">Afhending</div>
            <div className="mt-1 text-lg font-semibold text-zinc-900">Sækja eða fá sent</div>
            ...
            <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2">
              <button
                ...
                className={[
                  "rounded-2xl border px-4 py-4 text-left text-sm font-semibold",
              ...
              >
                ...
                >
                  Félagið afhendir vöruna á æfingu eða samkvæmt nánari upplýsingum.
                </div>
              </button>

              {SHIPPING_ENABLED_FOR_CLUB ? (
                <button
                  type="button"
                ...
                >
                  ...
                  >
                    Sending er valkvæð þjónusta sem félög geta síðar kveikt/slökkt á.
                  </div>
                </button>
              ) : null}
            </div>
            ...
            {deliveryMethod === "SHIP" ? (
              <div className="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div className="sm:col-span-2">
                  <Field label="Heimilisfang">
                    <Input
                      ...
                      placeholder="t.d. Laugavegur 1"
                    />
                  </Field>
                </div>

                <Field label="Póstnúmer">
                  <Input
                ...
                <div className="sm:col-span-2">
                  <Field label="Athugasemd fyrir afhendingu (valfrjálst)">
                    <Input
                      ...
                      placeholder="t.d. skilja eftir hjá dyrum"
                    />
                  </Field>
                </div>
              </div>
              ...
              <div className="mt-5 rounded-2xl bg-zinc-50 p-4 text-sm text-zinc-700">
                Varan verður sótt hjá félaginu. Seinna setjum við pickup-stað og
                afhendingarleiðbeiningar inn í stillingar hvers félags.
              </div>
            )}
          </div>
              ...
              {useGrant ? (
                <>
                  ...
                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    ...
                    <Field label="Upphæð" hint={`Hámark: ${formatISK(eligibleSubtotal)}`}>
